...
{  
  ...
  G4double setupX                     = 80.0*cm;
  G4double setupH                     = 50.0*cm;
  G4double rightBoardEnd              = wallsPlacement.x()+wallsHx-wallBoardThickness;
  G4double backBoardEnd               = wallsPlacement.y()+wallsHy-2*wallBoardThickness;
  G4double floorBoardEnd              = wallsPlacement.z()-wallsHz+floorBoardThickness;
  G4ThreeVector setupPlacement        = G4ThreeVector(rightBoardEnd,backBoardEnd-setupX,floorBoardEnd);
  G4RotationMatrix* setupRotation     = new G4RotationMatrix;
  setupRotation->rotateZ(M_PI/2);




   G4VSolid* det_solid                = new G4Box("det", 4.9*cm,4.9*cm,1.0*cm);
   G4LogicalVolume* det_logic         = new G4LogicalVolume(det_solid,Silicon,"det");
   G4RotationMatrix* detRot           = new G4RotationMatrix;
   detRot->rotateX(M_PI/2);
   detRot->rotateZ(3*M_PI/4);
   //Placing the detector as a daughter volume of the shielding did not work for some reason!
   new G4PVPlacement(G4Transform3D(*detRot,setupPlacement+G4ThreeVector(-setupX/4.,setupX/2.,setupH/2.0)),det_logic, "det", logicWorld, false, 0,checkOverlaps);   
   // Set det as scoring volume
   fScoringVolume = det_logic;

   G4LogicalVolume*   shieldingLog;
   fParser.Read("shielding.gdml");
   shieldingLog = fParser.GetVolume("shielding3_Aluminum.ast");
   shieldingLog->SetMaterial(Lead);
   shieldingLog->SetName("shieldingLog");
   new G4PVPlacement(G4Transform3D(*setupRotation,setupPlacement),shieldingLog, "shieldingPhys", logicWorld, false, 0,checkOverlaps);
